...
def analyze_papers(papers_data):
    if not config.GEMINI_API_KEY:
        ...
        return "Gemini API 키가 설정되지 않아 AI 분석이 생략되었습니다."
    ...
    # 🤖 [핵심 변경] HTML 금지! 오직 옵시디언 마크다운으로만 작성하도록 지시
    if config.BOT_NAME == "ai":
        ...
        3. 🚀 당장 시도해 볼 만한 "가성비" 후속 연구 아이디어(Research Gap): 위 트렌드를 종합하여, 수련병원 전공의가 당장 시도해 볼 만한 '가성비 높고 독창적인 임상 AI 후속 연구 아이디어 3가지'를 구체적으로 제안해 주세요.
        4. 🚨 형식 규정: 옵시디언(Obsidian)에 그대로 복사할 수 있도록 반드시 완벽한 **순수 마크다운(Markdown)** 문법(`#`, `**`, `-` 등)만 사용하여 작성하세요. 절대로 HTML 태그를 사용하지 마세요. (마크다운 코드 블록 기호 ```markdown 등도 쓰지 말고 텍스트만 출력하세요.)
        ...
        5. 🚨 형식 규정: 옵시디언(Obsidian)에 그대로 복사할 수 있도록 반드시 완벽한 **순수 마크다운(Markdown)** 문법(`#`, `**`, `-` 등)만 사용하여 작성하세요. 절대로 HTML 태그를 사용하지 마세요. (마크다운 코드 블록 기호 ```markdown 등도 쓰지 말고 텍스트만 출력하세요.)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 마크다운을 감싸는 불필요한 백틱(```markdown) 껍데기를 강제 제거하여 옵시디언 호환성을 높입니다.
            return safe_text.replace("```markdown\n", "").replace("```\n", "").replace("```", "").strip()
            ...
            if attempt < max_retries:
                ...
                return f"AI 분석 리포트 생성 중 오류가 발생했습니다. (최종 실패: {e})"
